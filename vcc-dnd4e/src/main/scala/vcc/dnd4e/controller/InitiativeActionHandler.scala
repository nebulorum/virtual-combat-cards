      ...
      if (itt.canTransform(firstp, action)) {
        ...
        action match {
          case Delay =>
            ...
            advanceDead()
          case MoveUp =>
            if (sequence.reserve.contains(cmb.id)) sequence.removeFromReserve(cmb.id)
            sequence.moveBefore(cmb.id, sequence.sequence.head)
          case ExecuteReady =>
            sequence.moveBefore(cmb.id, sequence.sequence.head)
            sequence.rotate
        ...
        throw new Exception("Error: Cant process " + (firstp, action) + " on state " + itt + " of " + cmb.id.name + " Sequence is: " + sequence.sequence)
      }
  }
  def advanceDead() {
