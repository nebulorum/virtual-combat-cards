...
class InitiativeSequenceTest extends TestCase {
  ...
  override def setUp() {
    ...
    mockTracker = new TrackerMockup[TrackerContext](new TransactionalProcessor(context) with ActionRecorder with TrackerControllerValidatingPublisher with InitiativeActionHandler with ActionAccumulator[TrackerContext] {
      def actionsExecuted: List[TransactionalAction] = actionsProcessed
  ...
  def startRound(who: Symbol) {
    val init = context.map(who).it.value
