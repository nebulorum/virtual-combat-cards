...
class CombatSequencerTest extends TestCase {
  ...
  def testExitReserve() {
    seq.removeFromReserve('f)
    assert((seq.sequence -- seq.reserve.toList) == List('a, 'b, 'c, 'd, 'f), (seq.sequence -- seq.reserve.toList))
    assert(seq.reserve.toList == List('e))
  }

  def testExitFromReserveSomeOneYouCant {
    seq.removeFromReserve('a)
    assert((seq.sequence -- seq.reserve.toList) == List('a, 'b, 'c, 'd), "Wrong sequence: " + (seq.sequence -- seq.reserve.toList) + " expected " + List('a, 'b, 'c, 'd))
    assert(seq.reserve.contains('e))
    assert(seq.reserve.contains('f))
  }
}
