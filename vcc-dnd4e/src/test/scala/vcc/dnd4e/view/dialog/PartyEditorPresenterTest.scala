...
class PartyEditorPresenterTest extends SpecificationWithJUnit with Mockito {
  ...
  private def entryFromEntityID(entityId: EntityID, quantity: Int = 1): PartyEditorView.PartyTableEntry = {
    PartyTableEntry(None, quantity, None, getEntityDefinition(entityId).name, getEntityDefinition(entityId).experience)
  }
