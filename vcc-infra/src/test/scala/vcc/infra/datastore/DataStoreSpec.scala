...
abstract class DataStoreSpec extends SpecificationWithJUnit {
  ...
  "a DataStoreBuilder" ->- (dsbContext) should {
    ...
    "not open a store that does not exists" in {
      adsb.open(storeID) must beNull
    }
    "create a store" in {
      adsb.create(storeID) must notBeNull
    }
    "prove it exists after creation" in {
      adsb.exists(storeID) must beTrue
    }
    "open a store that that exists" in {
      adsb.open(storeID) must notBeNull
    }
    "destroy the store" in {
      adsb.destroy(storeID) must beTrue
    }
    "prove the store does not exists after destroying" in {
      adsb.exists(storeID) must beFalse
    }
  }
