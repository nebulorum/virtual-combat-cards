 ...
 * return type. Users should display the windows using {promptUser} method.
...
abstract class ModalPromptDialog[T](window: Window, title: String) extends ModalFrame(window, title) {
  ...
  /**
   * Default ok Action which will correct result (using {collectResult}) and set the dialog invisible.
   */
   ...
   * @return Return the value from the dialog (see {dialogResult}).
   */
